...
- **5.5.11 — Input control + duplicate detection UI** (planned). Replace the inline new-client UI with a modal; live duplicate suggestions (lowercase-email exact, normalised-phone, `pg_trgm` name similarity above the configurable threshold); explicit "Create new anyway" confirmation; `quote_requests` → "Convert to client" action.
