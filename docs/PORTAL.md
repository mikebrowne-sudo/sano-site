...
- **5.5.7 — Customer + contractor polish** (in progress, branch `feat/portal-polish-phase-5-5-7`). Polish-only phase, no new systems. **Client dashboard**: section headers tightened to `Quotes` / `Invoices` / `Bookings`; empty states humanised ("You don't have any quotes yet"). **Contractor bottom nav**: `Training` tab renamed to `Onboarding` with a new `BookOpen` icon, re-pointed at the new `/contractor/onboarding` page; training stays accessible from there + `/contractor/profile`. **New `/contractor/onboarding`** read-only page: three sections (How we work, Expectations, Safety reminders) using bullet items, plus a card that links into the existing `/contractor/training` modules. **Job detail polish**: `ContractorJobActions` now flashes a brief confirmation banner after Start ("Job started — good luck.") and Complete ("Job marked complete. Nice work."), buttons bumped to `min-h-[52px]` for thumb-comfort, sticky-bar overlap unchanged. **Admin client audit**: `/portal/clients/[id]` gains a read-only "Activity" timeline reading `audit_log` for `entity_table='clients'`, mirroring the staff page; no filters, capped at 50 entries. **Settings**: `WorkforceSettings` extended with `invite_email_subject`, `invite_email_body_template`, `reset_email_subject`, `reset_email_body_template` with sensible defaults (templates use `{{name}}` / `{{link}}` placeholders) — defaults only, no UI yet, the underlying email helpers continue to use their inline templates until a future phase wires the overrides through. No RLS changes, no share-link changes, no new tables. Pending: PR + merge.

### Settings additions (planned, 5.5.2+)
Extend `workforce_settings.value` with: `invite_email_subject` (default seeded in 5.5.7), `invite_email_body_template` (default seeded in 5.5.7), `reset_email_subject` (default seeded in 5.5.7), `reset_email_body_template` (default seeded in 5.5.7), `invite_expiry_days` (default 7, planned), `enable_contractor_portal` (default true, ✅ live in 5.5.3), `enable_pwa_prompt` (default true, ✅ live in 5.5.4), `contractor_mobile_bottom_nav_enabled` (default true, ✅ live in 5.5.4), `enable_customer_portal` (default false, ✅ live in 5.5.5), `customer_invite_email_subject`, `customer_invite_email_body_template` (planned). Email body templates support `{{name}}` and `{{link}}` placeholders; the actual auth-invite helpers still use their inline copy until a future phase wires the override path through.
