...
- **5.5.6 — Customer access + clients RLS lockdown + basic dashboard** (in progress, branch `feat/customer-access-phase-5-5-6`). **RLS lockdown** (`docs/db/2026-04-27-phase-5-5-6-clients-rls-lockdown.sql` applied): drops the wide-open `Public can read clients for shared docs` policy; adds `clients_self_read` keyed off `auth_user_id = auth.uid()`; staff full-access preserved. Public share routes (`/share/quote/[token]/page.tsx`, `/share/quote/[token]/_actions.ts`, `/share/invoice/[token]/page.tsx`) refactored from the anon Supabase client to `getServiceSupabase()` so the joined `clients` SELECT keeps working; the `share_token` URL keys are still the only credential needed and the queries are constrained, so service-role bypass cannot be enumerated. **Customer invite flow** mirrors the contractor 5.5.3 pattern: new `src/app/portal/clients/[id]/_actions-access.ts` with `inviteClientUser` (gated on `enable_customer_portal`; persists `auth_user_id` from `generateLink`), `resendClientInvite` (alias), `disableClientAccess` (with reason), `enableClientAccess`, `markClientInviteAccepted`. `lib/auth-invites.ts` `redirectAfter` widened to include `'client'`. New components `ClientAccessActions` + `ClientAccessPanel` mounted on `/portal/clients/[id]` showing the 5-state status pill, last-invited / accepted timestamps, disabled banner + audit-logged actions. **Reset-password role chain** is now contractor → client → staff fallback; client users land on `/client/dashboard` after a successful password set. **Real `/client/dashboard`** replaces the 5.5.5 placeholder: read-only welcome card, recent quotes (with totals + share-link chevrons), recent invoices (same), and upcoming bookings (jobs scoped by `client_id`, status not in completed/invoiced/cancelled, scheduled_date >= today). The clients SELECT uses the user-scoped client (RLS-safe via `clients_self_read`); quotes/invoices/jobs use service-role but are filtered by the authenticated client's `client_id` only — no cross-customer leakage. When `enable_customer_portal=false`, the dashboard short-circuits to the "not available yet" message before any data fetch. New audit actions: `client.invite_sent`, `client.access_disabled`, `client.access_enabled`. Pending: PR + merge.

### Settings additions (planned, 5.5.2+)
Extend `workforce_settings.value` with: `invite_email_subject`, `invite_email_body_template`, `reset_email_subject`, `reset_email_body_template`, `invite_expiry_days` (default 7), `enable_contractor_portal` (default true, ✅ live in 5.5.3), `enable_pwa_prompt` (default true, ✅ live in 5.5.4), `contractor_mobile_bottom_nav_enabled` (default true, ✅ live in 5.5.4), `enable_customer_portal` (default false, ✅ live in 5.5.5), `customer_invite_email_subject`, `customer_invite_email_body_template` (planned with the customer invite UI in 5.5.6+).
