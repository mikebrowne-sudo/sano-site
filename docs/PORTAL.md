...
- **5.5.13 — Lifecycle enforcement + breadcrumb strip** (planned). Convert-to-job CTA on accepted quotes; enforce `job.client_id == quote.client_id` (now soft); breadcrumb strip `Quote → Job → Invoice` on detail pages; list-column updates per the design doc.
