...
**Primary focus (2026-04-30):** List-view UX consistency Phase 2 PR-B just shipped — Jobs list migrated onto the shared `PortalListTable` primitive (introduced in PR-A) with customer-first display, derived display status, value column, clickable Quote/Invoice chips, and cell-level navigation. All work since 5.5.14 is stacked on a single feature branch and not yet merged.
...
## Resume context — pick up after restart

Drop a new chat into `F:\Sano\01-Site` and point it at this section. Everything below reflects the live state on disk + remote as of **2026-04-30**.

### Where the work lives
- **Branch:** `feat/attention-bulk-phase-5-5-14` (pushed; latest commit `5948fc7`)
- **Stack since 5.5.14:** all phases below are **on this single branch, not yet merged to `main`**. Treat the branch as a long-running stack — every new phase rebases the prior work forward.
- **No PR open yet.** Open one at https://github.com/mikebrowne-sudo/sano-site/pull/new/feat/attention-bulk-phase-5-5-14 when ready.

### What's stacked on the branch (most-recent first)
1. **Airbnb option in client business_type** (✅ committed `5948fc7`, applied via Supabase MCP)
2. **List-view UX consistency Phase 2 PR-B — Jobs onto PortalListTable** (✅ committed `e34adaf`)
3. **List-view UX consistency Phase 2 PR-A — PortalListTable primitive + Invoices migration** (✅ committed `bec6f79`)
4. **List-view UX consistency Phase 1 follow-up — invoice search + detail linked records** (✅ committed `0675885`)
5. **List-view UX consistency Phase 1 — Invoices parity** (✅ committed `3ea9e58`)
6. **Quote source-of-truth + flow clarity** (✅ committed `ad72e0f`)
7. **Residential pricing — Win/Standard/Premium tiers + override hours UI + validation** (✅ committed `b108467` + `bea1064` validator split-out)
8. **Residential pricing tweaks — consistency follow-up** (✅ committed `4177458`)
9. **Residential pricing engine — settings + override + cap formula** (✅ committed `ae14d26`)
10. **Residential service upgrade — wording engine + extras taxonomy** (✅ committed `d1e5871`)
11. **Invoice double-count fix from createInvoiceFromJob** (✅ committed `4559bab`)
12. **5.5.16 — Workflow simplification + action-driven status + data integrity**
13. **5.5.14 v2 — Cleanup-mode access control + admin gating**
14. **5.5.14 — Bulk actions + attention system + UX polish**
15. **5.5.12 — Quote-accept → Job Setup wizard + residential scope expansion**
16. **5.5.11 — CRM input control + client payment setup**

Full per-phase detail (DB migrations, file paths, what shipped, what's deferred) is in the **In Flight** sections below — each item retains its full description.

### Phase 3 (next, not yet started)
The agreed Phase 3 of the list-view UX work is the **expanded next-action matrix** — moving the per-row "next step" hint out from under the attention chips into a richer, action-driven affordance. Not started; no plan written yet.

### Skills + workflows in use
- **`superpowers:brainstorming`** — every non-trivial change starts with a design pass: explore → propose 2-3 approaches → present design in sections → get approval → write spec to `docs/superpowers/specs/`.
- **`superpowers:executing-plans`** — execute approved plans step-by-step, marking tasks complete as we go (not batched at the end).
- **`fewer-permission-prompts`** — allowlist tuning when permission prompts pile up.
- **`superpowers:using-git-worktrees`** — referenced when an isolated workspace is needed; not currently in use (working directly on the feature branch).

### Codebase patterns we lean on (see existing code for examples)
- **`'use server'` discipline:** action files export only async functions. Types and sync helpers live in companion `_lib-*.ts` files (e.g. `_actions-job-setup.ts` + `_lib-job-setup.ts`). Breaking this rule fails the Netlify build.
- **Display-settings registry:** `JOB_FIELDS` / `QUOTE_FIELDS` / `INVOICE_FIELDS` in `src/lib/portal-display-settings.ts` drive what columns render. Defaults merged via `mergeEntity` so old `portal_settings` rows fall through cleanly.
- **`BulkSelectProvider` pattern:** the page wraps the list primitive with `BulkSelectProvider` so the sticky action bar mounts at page level; the primitive only renders the header + per-row checkboxes when `canCleanup` is true.
- **`PortalListTable` primitive** (`src/app/portal/_components/PortalListTable.tsx`): generic server component shared by Invoices and Jobs; Quotes still on its own renderer (intentional — Quotes was not migrated in Phase 2; can be added when there's appetite). Page owns data + `cell()` dispatch; primitive owns table shell + mobile cards + bulk-select wiring + attention chips placement + "Open →" affordance + optional `rowExtraActions` slot.
- **Cell-level navigation:** every list row is built from independent `<Link>` cells (job number, linked quote chip, linked invoice chip, "Open →") rather than a single whole-row `<Link>`.
- **Derived display status** (`getJobStatus()` in `src/lib/job-status.ts`): UI shows operationally-honest labels (Needs scheduling / Scheduled / In progress / Completed / Invoiced); DB `jobs.status` continues to store the explicit transitions. Same pattern available for invoices (`computeInvoiceDisplayStatus` promotes `sent` → `overdue` past due date).
- **Customer-first label rule:** B2B identity wins — `clients.company_name` if set, fall back to `clients.name`. Used in Jobs list row mapper; Quote / Invoice lists still show client name (operator can flip via display settings).
- **PostgREST embedded relations:** `source_quote:quotes!quote_id ( id, quote_number )` style with FK alias to pin the relationship. Embeds return arrays for non-1:1; we pick `[0]` defensively. Always include `id` on embeds when the chip needs to deep-link.
- **Side-query for cross-table search:** PostgREST `or` can't filter on embeds. Pattern: pre-resolve matching IDs (e.g. quote IDs by quote_number) with a small separate query and fold them into the main `OR` clause as `quote_id.in.(...)`.
- **Supabase MCP for migrations:** apply via `mcp__claude_ai_Supabase__apply_migration`; **also** save a canonical SQL file at `docs/db/<YYYY-MM-DD>-<name>.sql` so the migration is replay-able.

### Verify-before-claim rules baked in
- **"Live" = deployed + verified** — code committed is "implemented", not "live". Mark items live in this brief only after Netlify deploy + manual verification.
- **Always end portal tasks with a Commands block** — local run cmd, git commit+push, suggested commit message, explicit "no command" note.
- **Never insert into `auth.users` directly** — Supabase Auth API or Dashboard only.
- **Never read `.env*` files** — even redacted reads not trusted; use `netlify env:list` or verbal confirmation.
- **Never use bracketed paths in `git add` on Windows cmd** — `[id]` paths silently match nothing; use `git add -A` or `:(literal)` prefix.

### Verification commands (run before commit)
```
npx tsc --noEmit
npx next lint --dir <touched dirs>
npx jest src/lib/__tests__/        # expect 253/253
```

### To resume in a new chat, paste this:
> "Working in `F:\Sano\01-Site` on branch `feat/attention-bulk-phase-5-5-14`. Master brief at `docs/PORTAL.md` — read the **Resume context** section first. Last commit: `5948fc7` (Airbnb business_type option). Most recent shipped phase: list-view UX consistency Phase 2 PR-B (Jobs onto PortalListTable). Next agreed work: list-view UX consistency Phase 3 (expanded next-action matrix). The feature branch carries all phases since 5.5.14 stacked together; no PR is open yet."
...
- **5.5.3 — Contractor portal access + staff tweaks** (✅ shipped, merged to main). Migration `docs/db/2026-04-26-phase-5-5-3-contractor-access.sql` applied: `contractors` gains `invite_accepted_at`, `access_disabled_at`, `access_disabled_reason` (mirroring staff); index `contractors_auth_user_idx`. Staff RLS extended with `staff_all_read` so any authenticated staff can read the team list (writes still admin-only). New `_actions-access.ts` server actions: `inviteContractorUser` (gated on `enable_contractor_portal` setting; persists `auth_user_id` from `generateLink` response), `disableContractorAccess` (with reason), `enableContractorAccess`, `markContractorInviteAccepted` (called from reset-password page after the staff fallback). New components `ContractorAccessActions` + `ContractorAccessPanel`: 5-state status pill (`not_invited` / `invited` / `active` / `disabled` / `feature_disabled`), action buttons (Send/Resend invite, Disable with reason modal, Re-enable), 44px tap targets. Contractor detail page: replaces the old read-only Portal access section with the new panel. Contractor list page: adds Access column (desktop) + chip (mobile) computed via `contractorAccessStatus()`. Reset-password form now calls `markContractorInviteAccepted` when the user matches a contractor row. Staff InviteActions gain a "Access is disabled. Contact admin if this is unexpected." callout when status is disabled. Workforce settings extended with `enable_contractor_portal` (default true). Audit actions: `contractor.invite_sent`, `contractor.access_disabled`, `contractor.access_enabled`.
- **5.5.4 — Mobile-first contractor portal + PWA** (✅ shipped, merged to main). PWA setup: `public/manifest.json` (name "Sano", standalone display, `start_url=/contractor/jobs`, theme `#076653`, background `#F7F9F7`), root `app/layout.tsx` extended with `metadata.manifest`, `metadata.icons`, `appleWebApp`, and a `viewport.themeColor` export. `ContractorBottomNav` (mobile-only, `md:hidden`, fixed bottom, safe-area-inset aware) with three tabs Jobs / Training / Profile and `aria-current` active state. Contractor layout pads `<main>` with `pb-24 md:pb-6` to clear the nav. Jobs list (`/contractor/jobs`) re-grouped into Today / Overdue / Upcoming / Earlier buckets — Today gets emerald ring, Overdue amber ring; cards show date+time and truncated address with calendar/map icons; refreshed empty state. Job detail (`/contractor/jobs/[id]`): primary action wrapper switches to `fixed` on mobile (sat above the bottom nav at `calc(56px + env(safe-area-inset-bottom))` with white/blur background) and falls back to inline on `md+`; page wrapper bumps to `pb-28 md:pb-8` so the last card isn't covered. New `/contractor/profile` page (read-only contact card + Training shortcut + sign-out button). `InstallPrompt` component: A2HS captures `beforeinstallprompt` on Android/Chrome and offers an Install button; iOS Safari sees a "Share → Add to Home Screen" hint instead; dismiss persists in localStorage; hidden when already running standalone. Workforce settings extended with `enable_pwa_prompt` (default true) and `contractor_mobile_bottom_nav_enabled` (default true).
- **5.5.5 — Customer-portal scaffold + PWA icon tidy-up** (✅ shipped, merged to main). Migration `docs/db/2026-04-27-phase-5-5-5-customer-portal-scaffold.sql` applied: `clients` gains `auth_user_id` (FK → auth.users on delete set null), `invite_sent_at`, `invite_accepted_at`, `access_disabled_at`, `access_disabled_reason`; partial indexes `clients_auth_user_idx` and `clients_email_lower_idx`. Workforce settings: `enable_customer_portal` (default **false**). Middleware gains `/client/:path*` matcher with role gating. Placeholder `/client/dashboard` page renders one of two messages depending on the flag. PWA icon tidy-up: `scripts/generate-pwa-icons.mjs` (sharp dev-dep) generated `public/icons/icon-{192,512,maskable-512}.png`; manifest + apple-touch icon re-pointed.
- **5.5.6 — Customer access + clients RLS lockdown + basic dashboard** (✅ shipped, merged to main). RLS lockdown migration applied: drops `Public can read clients for shared docs`; adds `clients_self_read` (`auth_user_id = auth.uid()`); staff full-access preserved. Public share routes (`/share/quote/[token]/page.tsx`, `/share/quote/[token]/_actions.ts`, `/share/invoice/[token]/page.tsx`) refactored to `getServiceSupabase()` so the joined `clients` SELECT keeps working with no public-read policy. Customer invite flow mirrors contractor 5.5.3: `src/app/portal/clients/[id]/_actions-access.ts` (invite/resend/disable/enable/markAccepted), `ClientAccessActions` + `ClientAccessPanel` mounted on `/portal/clients/[id]`. Reset-password role chain widened to contractor → client → staff. `/client/dashboard` replaced with real read-only view: recent quotes/invoices + upcoming bookings, all scoped to the authenticated client's `client_id`. Audit actions: `client.invite_sent`, `client.access_disabled`, `client.access_enabled`.
- **5.5.7 — Customer + contractor polish** (✅ shipped, merged to main). Section headers tightened on `/client/dashboard`; empty-state copy humanised. Contractor bottom nav renamed Training → Onboarding (BookOpen icon) and re-pointed at the new read-only `/contractor/onboarding` page (How we work / Expectations / Safety reminders). `ContractorJobActions` gains a brief success flash after Start / Complete; tap targets bumped to `min-h-[52px]`. `/portal/clients/[id]` gains a read-only Activity timeline reading `audit_log` for `entity_table='clients'`. `WorkforceSettings` extended with `invite_email_subject`, `invite_email_body_template`, `reset_email_subject`, `reset_email_body_template` defaults (templates support `{{name}}` / `{{link}}`).
- **5.5.8 — Settings UI + email template wiring + client PDF actions** (✅ shipped, merged to main via PR #100). `lib/auth-invites.ts` loads the four email-template fields via `loadWorkforceSettings(getServiceSupabase())` and passes them to the email helpers as `subjectOverride` / `bodyTemplateOverride`; `lib/resend.ts` adds `applyTemplate` + `templateToHtml` and the `authEmailFrame` now takes `bodyHtml`. Admin settings page at `/portal/settings/portal-access` (KeyRound card on the hub) saves feature flags + email templates to `workforce_settings`. `/client/dashboard` adds View / PDF buttons; new `AutoPrint` client island fires `window.print()` when `?print=1`.
- **5.5.9 — CRM data foundation (additive + backfill)** (✅ shipped, merged to main via PR #101). **Schema** (`docs/db/2026-04-27-phase-5-5-9-crm-foundation-schema.sql` applied): `clients` gains `client_type` (`individual` / `company` / `property_manager`), `lead_source` (`google` / `social` / `referral` / `existing_client` / `other`), `referred_by`, `accounts_email`, `is_archived` (default false), `archived_at`. New `contacts` table (`client_id` FK, `full_name`, `contact_type` ∈ `primary` / `accounts` / `site` / `other`, `email`, `phone`, `notes`, `created_at`) with `Staff full access` + `contacts_self_read` RLS. New `sites` table (`client_id` FK, `address`, `suburb`, `access_notes`, `parking_notes`, `alarm_notes`, `default_contact_id` FK → contacts) with the same two policies. Document tables gain nullable links: `quotes.contact_id`, `quotes.site_id`; `jobs.contact_id`, `jobs.site_id`, `jobs.source` (`quote` / `manual` / `recurring`); `invoices.contact_id`, **`invoices.job_id`** (the missing reverse link), `invoices.source` (`job` / `manual` / `recurring`). Existing snapshot columns (`contact_*`, `accounts_*`, `service_address`, `address`) untouched — historical PDFs read identically. **Backfill** (`docs/db/2026-04-27-phase-5-5-9-crm-foundation-backfill.sql` applied): every existing client got a primary contact and a primary site (resolution order: `clients.service_address` → most-common `quotes.service_address` → most-common `jobs.address`); `clients.accounts_email` filled from the most-frequent non-blank value across that client's quotes + invoices, and an `accounts` contact created when the email differs from the primary; quotes/jobs/invoices linked to each client's primary contact + site (only when null); jobs gained `source` (8 `quote`, 2 `manual`); invoices gained `job_id` (7 unambiguous reverse links from `jobs.invoice_id`) and `source` (7 `job`, 10 `manual`); `clients.client_type` defaulted to `company` when `company_name` was set, else `individual`. Backfill is idempotent and never overwrites a non-null value. **Soft validation warning surfaced**: 1 job (`JOB-0002`) has `client_id` ≠ `quote.client_id` — flagged for the 5.5.10 cleanup pass, **not auto-fixed**. No UI changes in this phase. No existing data deleted, renamed, or rewritten.
- **5.5.10 — CRM cleanup tools** (✅ shipped, merged to main via PR #102 + hotfixes #103 + `fix/clients-detail-defensive`). Strict-rules **archive / safe-delete / merge** for clients, plus light data-fix tools that resolve every flag the cleanup dashboard surfaces. New server actions in `src/app/portal/clients/_actions-cleanup.ts`: `archiveClient`, `unarchiveClient`, `safeDeleteClient` (refuses when the client has any quote/job/invoice — message: "Archive instead."), `mergeClients` (moves `quotes`, `jobs`, `invoices`, `contacts`, `sites` from source → target via FK update; archives the source; audited as `client.merged` with `before/after` and a `moved` count summary), `getClientLinkCounts`, `findPossibleDuplicates` (case-insensitive email + name exact, normalised-digits phone), `fixJobClientToMatchQuote`, `unlinkJobQuote`, `linkInvoiceToJob`. New `ClientCleanupActions` component on `/portal/clients/[id]` exposes the three buttons + a merge modal with a typeahead-filtered candidate list and a confirmation summary (e.g. "5 quotes, 1 job, 3 invoices, 2 contacts, 1 site will move"). Replaces the old generic `<DeleteButton type="client" />`. New "Possible duplicates" panel on the same page lists matches with the matched-on attribute. Client list (`/portal/clients`) gains a "Show archived" / "Hide archived" toggle (URL `?archived=1`), per-row `Archived` chip + `Possible duplicate` chip; archived rows get `opacity-60`. **Selectors filter `is_archived = false`** in `/portal/quotes/new`, `/portal/jobs/new`, `/portal/jobs/[id]/edit`, `/portal/recurring-jobs/new`, `/portal/recurring-jobs/[id]/edit` — archived clients no longer show up when creating new documents but stay readable on existing records. New **`/portal/cleanup`** dashboard (admin-only, linked from the settings hub via a new AlertTriangle card) shows four sections: duplicate clients (grouped by email or normalised phone), job ↔ quote client mismatches, invoices with no `job_id`, and manual jobs (`quote_id` is null). New `JobMismatchBanner` (banner + "Set client to match quote" / "Unlink quote" actions) on `/portal/jobs/[id]`; new `InvoiceLinkBanner` (banner + searchable job picker scoped to the same client) on `/portal/invoices/[id]`. `WorkforceSettings` extended with `enable_client_merge`, `enable_client_delete`, `enable_cleanup_dashboard` (defaults true; not yet exposed in the settings UI). New audit actions: `client.archived`, `client.unarchived`, `client.deleted`, `client.merged`, `job.fixed`, `invoice.linked`. **No data is auto-merged or auto-fixed** — every action is explicit, audited, and (for merge) reversible by un-archiving the source.

  **Hotfix notes (post-merge):**
  - **PR #103** — `_actions-cleanup.ts` had `'use server'` plus two `export interface` declarations. Next.js 14 only allows async-function exports from a "use server" module; the non-function exports crashed `/portal/clients/[id]` with `Application error` (digest 3118903231). Fix: read-only helpers + types moved to a new `_lib-cleanup.ts` (no `'use server'`).
  - **`fix/clients-detail-defensive`** — A second class of crashes (digest 2822761555) came from a single `Promise.all` where any one of the five parallel reads could throw and bring the whole render down. Each branch is now isolated in its own try/catch with safe fallbacks; `_lib-cleanup` helpers wrap each Supabase round-trip and tolerate per-table failures inside the count fan-out. The phone-match query swapped `.not('phone', 'is', null)` chained on a complex builder for an in-JS filter (cheaper at this row count, removes a runtime quirk). Page null-guards every cast off the loaded client (`name` falls back to "Unnamed client"; settings reads use optional chaining for the customer-portal flag). `/client/dashboard` `firstName` derivation hardened so a null client name never crashes the welcome card.
  - **No schema or data cleanup required** from either bug — both fixes are pure code-shape hardening. The 5.5.9 backfill state is intact; the cleanup dashboard continues to surface the genuine outstanding items (currently 5 unlinked invoices; the previously-flagged JOB-0002 mismatch is now resolved by archive, so it correctly drops out of the active mismatch list).

### CRM roadmap (after 5.5.10)
- **5.5.11 — CRM input control + client payment setup** (✅ shipped, merged to main). Migration `docs/db/2026-04-28-phase-5-5-11-clients-payment-setup.sql` applied: `clients` gains `payment_type` (`prepaid` / `on_account`), `payment_terms` (`7_days` / `14_days` / `20_of_month` / `custom`), `business_type` (`property_management` / `construction` / `office_commercial` / `retail_hospitality` / `body_corporate` / `other`). Existing `quotes.payment_type` / `invoices.payment_type` columns (`cash_sale` / `on_account`) are unchanged — the new-quote form maps `clients.payment_type='prepaid'` → `quotes.payment_type='cash_sale'` so PDFs / share routes / Stripe checkout / proposal templates read identically. New `src/app/portal/clients/_components/NewClientModal.tsx` — single 6-section form (Client / Primary contact / Accounts contact / Payment setup / Lead source / Notes) with smart payment defaults (Individual → prepaid; Company → on_account 14 days; Property management → on_account 20th of month) and **live duplicate detection** (lowercase email exact + normalised digits-only phone exact + `ilike` name substring, capped at 5; debounced 350 ms). Duplicate panel offers "Use existing" per match plus a single "Create anyway" confirm checkbox; the submit button is blocked until either a match is selected or the override is checked. New server action `createClientWithSetup` in `src/app/portal/clients/_actions-setup.ts` creates the client + a `primary` contact + an optional `accounts` contact (only when distinct from primary) + an optional first site (only when an address was supplied) in a single call. `findClientMatches` exposes the dedupe lookup. Two render modes: standalone `/portal/clients/new` page renders the modal `inline`; the new-quote form's existing-client tab gains a "+ New client" button that pops the overlay and pre-selects the new client on save. The legacy "Quick (no save)" tab is retained as a fallback for ad-hoc snapshots that don't need a persisted client. Quote form payment-section adds a small read-only "Inherited from client" banner showing the client's payment setup; the existing per-quote payment-type buttons remain as a manual override. **Deferred to 5.5.12**: site/contact picker (every client currently has exactly one of each post-5.5.9 backfill — picker becomes useful only when clients accumulate more), the accepted-quote → job-setup wizard, and the residential scope expansion (occupancy / pets / parking / stairs / condition / access). Pending: PR + merge.
- **5.5.12 — Quote-accept → Job Setup wizard + residential scope expansion** (in progress, branch `feat/job-setup-wizard-phase-5-5-12`). Migration `docs/db/2026-04-28-phase-5-5-12-residential-scope-expansion.sql` applied: `quotes` and `jobs` both gain nullable text columns `occupancy`, `pets`, `parking`, `stairs`, `condition_level`; `quotes.access_notes` added (jobs already has `access_instructions`). None of the new columns feed the pricing engine — they're operational hints captured in the wizard and snapshotted into `jobs.scope_snapshot`. New 5-section **JobSetupWizard** modal (Schedule / Site & contact / Worker / Payment / Notes & scope) opened by `CreateJobButton` on accepted quotes; replaces the previous one-click path that created a half-empty draft job. Captures `scheduled_date`, `scheduled_time`, `duration_estimate`, `allowed_hours` (defaults to `quote.estimated_hours`), `address`, `access_instructions`, optional `contractor_id` + per-job hourly override, payment-exception path with required reason when prepaid, plus the 5 scope hints + `internal_notes`. **Worker assignment readiness gate**: `listAssignableContractors` returns every non-archived contractor with a derived status (`active` / `pending_onboarding` / `inactive`); the dropdown disables non-active rows with a clear "Not ready for job assignment — Complete onboarding and trial requirements first" hint. Active rows show a small admin-internal cost readout (Client price / Allowed hours / Worker rate / Estimated labour cost / Estimated margin) — never sent to the contractor portal. New server action `createJobFromQuoteWithSetup(quoteId, setup)` in `src/app/portal/quotes/[id]/_actions-job-setup.ts` writes the job with the captured fields and **honours quote payment_type into job.payment_status**: `cash_sale` (prepaid, no override) → `status='draft'` + `payment_status='awaiting_payment'`; `cash_sale` + override → `payment_status='exception'` (override reason audit-logged); `on_account` → `payment_status='on_account'`. Quote → `converted` transition + audit (`quote.converted_to_job`) match the legacy path so the workflow bar / proposal lock / share-link logic are untouched. The legacy `createJobFromQuote` action is retained for fallback callers but no UI invokes it after this phase. Pricing engine, commercial-quote flow, share routes, PDFs, emails, and Stripe checkout: all untouched. **Deferred** (out of scope for this phase, called out in the brief output): exposing the new scope fields on the QuoteBuilder UI itself (today they only flow in via the Job-Setup wizard); a dedicated `/portal/jobs/[id]` UI panel for the new scope fields beyond the existing details. Pending: PR + merge.
- **Quote source-of-truth + flow clarity** (in progress, same branch). Pulls quote → job → invoice into a single navigable thread, surfaces linked records on every list view, and centralises invoice date logic. **Conversion guard upgraded** (`src/lib/quote-conversion-guard.ts`): when `assertQuoteConvertible` rejects because a downstream record already exists, it now returns `{ error, existing: { kind, id, number } }` so the UI can render an "Open existing job/invoice" CTA instead of a dead-end text error. The already-converted branch also looks up the linked record so legacy "this quote is converted" rejections gain the same affordance. Forwarded through `convertToInvoice`, `createJobFromQuoteWithSetup`, and `createJobAndInvoiceFromQuote`. **JobSetupWizard** consumes the new field and renders an "Open existing job → " button under the error block. **Quote list** (`/portal/quotes`) now pulls the linked job + invoice for every quote on the page (not just accepted ones — converted quotes also need the badges). New `Map<quote_id, {job}> + Map<quote_id, {invoice}>` structures power the row badges; the previous attention-rule boolean sets are derived from the same maps. Each row shows `Job JOB-XXXX · status` and `Invoice INV-XXXX · status` chips below the attention strip when the records exist. **Quote detail** (`/portal/quotes/[id]`) gains a `<QuoteLinkedRecords>` strip rendered above the `QuoteWorkflowBar` whenever a job or invoice is linked — clickable rounded badges showing the number, a coloured status pill, and the next relevant date (scheduled date for jobs, due date for invoices). Visible regardless of quote status, so a converted quote isn't a dead-end. **Jobs list** (`/portal/jobs`) joins `source_quote:quotes!quote_id (quote_number)` and `linked_invoice:invoices!invoice_id (invoice_number, status)` so each row can show "From QUO-XXXX" + "Invoice INV-XXXX · status" chips. **Jobs search** now finds records by quote number — a side-query resolves matching `quote_id`s and folds them into the `OR` clause alongside the existing job-field matches. Empty result → no `quote_id.in.()` appended, so the existing matches still apply. **Schedule-change audit trail** in `updateJob`: a snapshot of the previous `scheduled_date` + `scheduled_time` is taken before the update; when either field actually moves, an `audit_log` row with `action = 'job.schedule_changed'` is written with `before` / `after`. No-op writes (re-saving the same value) produce no audit entry. **Invoice date logic centralised** in `src/lib/invoice-dates.ts`: `resolveServiceDate({job_completed_at, job_scheduled_date, quote_scheduled_clean_date})` returns the canonical service date (job dates win over the original quote date — re-scheduled jobs invoice for the latest date). `computeInvoiceDueDate({payment_type, payment_terms, date_issued, service_date})` is the single source of truth for due dates, replacing the inline cash-sale / on-account branches that used to live in `convertToInvoice` only. New `payment_terms` keys honoured: `due_immediately`, `due_on_completion`, `7_days`, `14_days`, `20_of_month` (rolls to next month when issued is past the 20th). Falls through to the `payment_type` axis when no explicit terms are set, preserving the previous on-account 14-day default. **createInvoiceFromJob** now consumes both helpers — pulls `clients.payment_terms` + the source quote's `payment_type`, computes the service date from `completed_at ?? scheduled_date`, and writes a non-null `due_date` when the inputs allow (still null when `date_issued` is null, which is the normal "draft until send" state). **convertToInvoice** rewritten to call the same helpers — also consumes `clients.payment_terms`, replacing the old hardcoded 14-day branch. `quotes.payment_type` precedence preserved. **Tests**: 15 new in `invoice-dates.test.ts` covering every payment-terms branch + the 20th-of-month roll-over + the cash_sale day-before-service case + null-handling. 175/175 across all five focused lib suites. Pre-existing 5 stale 7-bed-clamp tests in the legacy `quote-pricing.test.ts` — already addressed in the prior phase. Untouched: invoice total formula, share routes, PDF render, email render, Stripe checkout, commercial pricing, status-mode UI. Pending: PR + merge + production validation.
- **Residential pricing — Win/Standard/Premium tiers + override hours UI + validation** (in progress, same branch). Tightening pass that adds explicit pricing tiers, exposes override hours in the UI, and locks down settings with inline validation. **DB:** `docs/db/2026-04-29-residential-pricing-tiers-seed.sql` extends the seeded jsonb singleton with `win_hourly_rate=70`, `standard_hourly_rate=75`, `premium_hourly_rate=80`, `default_pricing_tier='standard'`. Idempotent jsonb merge — leaves the rest of the row alone. Applied via MCP. **Settings shape:** `ResidentialPricingSettings` gains `win_hourly_rate`, `standard_hourly_rate`, `premium_hourly_rate`, `default_pricing_tier`. `default_hourly_rate` is preserved as a deprecated alias for `standard_hourly_rate` (kept in sync by the loader + form) so legacy callers / breakdown consumers don't break. Loader resolves win/premium with sensible offsets when only `default_hourly_rate` exists on a legacy row. **Engine:** `calculateQuotePrice` now picks the rate from `settings.{tier}_hourly_rate` keyed by the `mode` argument; falls back to `settings.default_pricing_tier` when the requested mode isn't priced; final safety net is `settings.default_hourly_rate`. The Phase residential-pricing-engine band-aid (offset arithmetic on top of `default_hourly_rate`) is gone — explicit per-tier rates replace it. **Settings UI:** "Hourly rate + service fee" section replaced with "Pricing tiers + service fee" — three rate inputs (Win / Standard / Premium) plus a default-tier select and the service-fee field. Inline validation: tier rates must be > 0 and ordered Win ≤ Standard ≤ Premium; service multipliers ∈ [0.5, 2.5]; condition multipliers ∈ [0.5, 2.5]; `condition_multiplier_cap` ∈ [1.0, 2.0]; addon hours ≥ 0; service fee ≥ 0. Errors render under each input + a banner above Save; Save is disabled until issues clear. Server action `saveResidentialPricingSettings` runs the same validator and refuses invalid payloads (UI hides are convenience, not security). New exported `validateResidentialPricingSettings(settings) → ValidationIssue[]`. **Quote UI:** `PricingSummary` swaps the legacy "Pricing mode" wording for "Pricing tier" with descriptive hints (Win the work — More competitive rate; Standard — Balanced margin; Premium — Higher margin / detail jobs). Adds a Rate row showing the active tier label + hourly figure. Stat columns now read "Estimated" and "Final" (was "Calculated" / "Final"); the override badge text is "Adjusted from estimated values". **Override hours UI:** `OverridePanel` gains a second numeric input alongside the price — `override_hours` (optional). Empty means "use the engine's estimate"; a value flows through `createQuote` to `quotes.override_hours` (column added in the prior phase) and inherits as `quotes.allowed_hours` for the job-setup wizard. **Internal-only badge** added to the breakdown summary so anyone glancing at the screen knows it isn't customer-facing. **Breakdown:** rate row labels the tier explicitly; Estimated → Final lines collapsed cleanly when no override is active. **Stale 7-bedroom test cleanup**: the old 5-failure scenario expecting clamping at 6 beds was rewritten — engine has supported 7 beds since `b50c45f` (9.0-hour base row). Tests now assert `bed_count_used = 7`, `base_hours = 9.0`, `final_hours = 10.0`, `calculated_price = $775.00`. New companion scenario verifies `bedrooms: 8` clamps at 7. **Canonical Win/Premium scenarios** updated to reflect new rates ($70 / $80) — five canonicals + the End-of-Tenancy/Win + Deep+Premium scenarios. Standard quotes are mathematically unchanged. **Test results**: 152/152 in the three relevant suites; one type-error in `OverridePanel.test.tsx` and one assertion update fixed for the new field. No invoice / share / PDF / Stripe / commercial-pricing surface changed. Pending: PR + merge + production validation.
- **Residential pricing tweaks — consistency follow-up** (in progress, same branch). Tightening pass on the residential pricing engine, no rebuild. **EditQuoteForm wired to settings**: the quote-detail page now loads `loadResidentialPricingSettings(supabase)` alongside the commercial settings and forwards them through to `EditQuoteForm`. Both `isPricingEligible` and `calculateQuotePrice` calls inside the form now pass `residentialPricingSettings` so an edited quote prices identically to a new quote (and the locked-quote guard — `if (!eligible || isLocked) return null` — still prevents recalculation of accepted/converted/declined rows). **Settings row seeded**: `pricing_residential_settings` is no longer `{}`; the row now holds the full settings object so admins see populated defaults instead of empty fields. `docs/db/2026-04-28-residential-pricing-settings-seed.sql` committed as the canonical replay. **Two default normalisations applied to fallback + seed**: `average_condition` 1.10 → **1.00** ("average" should be neutral, not an automatic uplift); `airbnb.turnover` 0.9 → **1.0** (presentation expectations remain — should not auto-discount). Two pricing tests updated to match the new behaviour: the 1-bed Airbnb scenario now lands exactly on the minimum-hours floor (still rounds to 2.5 hrs / $212.50) and the multiplicative-min weekly scenario keeps `min_applied=true` because the weekly discount still drops the new 2.0-hour result below the floor. **No price change for already-stored quotes** — the engine only recalculates when an editable form re-runs the engine; sent / accepted / converted / declined quotes retain their stored breakdown. Pending: PR + merge + production validation.
- **Residential pricing engine — settings + override + cap formula** (in progress, same branch). Brings the residential pricing engine under admin control without changing any prices for existing quotes. **DB:** `docs/db/2026-04-28-residential-pricing-settings.sql` creates the `pricing_residential_settings` jsonb singleton (key='default') with admin-only RLS, plus adds nullable `quotes.override_hours` (numeric). Both applied via MCP. **Settings shape**: `default_hourly_rate`, `service_fee`, `minimum_job_hours`, `buffer_standard`/`buffer_heavy`, `heavy_buffer_service_types[]`, `rounding_step_hours`, `base_hours_by_bedroom` (1..7), `bathroom_extra_hours`, `high_use_extra_hours`, `service_multipliers` (per "category.code"), `condition_multipliers` (per tag), `condition_multiplier_cap` (default 1.45), `addon_hours` (per addon code). All fallback values are bit-for-bit identical to the prior in-code constants → zero pricing change for any existing quote. **`src/lib/residentialPricingSettings.ts`** — loader + `FALLBACK_RESIDENTIAL_PRICING_SETTINGS`; never throws (every key falls back to constants on missing/malformed data). **Engine refactor (`src/lib/quote-pricing.ts`)** — `calculateQuotePrice(input, mode, override?, settings?)` now reads every knob from settings; the legacy module-level constants (HOURLY_RATES, SERVICE_FEE, MIN_JOB_HOURS, BUFFER_STANDARD, BUFFER_HEAVY) remain as exports mirroring the fallback so any caller still importing them keeps working. The breakdown object gains `condition_cap_applied` (boolean) and `settings_signature` (stable digest of the active settings) for diagnostics. **Multi-tag condition combine** is now the capped formula `min(cap, m1 + 0.5 × Σ(m_i − 1.0) for i ≥ 2)` — single-tag and zero-tag quotes are mathematically identical to before; multi-tag quotes get capped at 1.45 (default) instead of compounding multiplicatively. The `quote-pricing.test.ts` "Condition adjustments stack multiplicatively" scenario was rewritten to assert the new capped behaviour (3.5×1.6×1.25 + 0.5 bath → ceil 8.5 → $662.50 instead of the old multiplicative $700). **Hourly rate**: settings.default_hourly_rate wins when settings is passed; the legacy mode selector (win / standard / premium) still works via constant offsets so the existing UI keeps doing something. When no settings argument is passed, the engine resolves rate from HOURLY_RATES[mode] exactly as before — zero regression. **Settings UI**: `/portal/settings/pricing-engine` now hosts two tabs (Commercial / Residential) via new `PricingEngineTabs` component. New `ResidentialPricingForm` is a compact one-page editor for every knob with a "Reset to defaults" button. Server action `saveResidentialPricingSettings` upserts the singleton + audits as `residential_pricing_settings.updated`; `resetResidentialPricingSettings` restores the fallback. Both gated on michael@sano.nz. **`override_hours` field on quotes**: persisted on create when supplied; flows into `quotes.allowed_hours` as the default (so a job-setup wizard inherits the operator-set hours when an override is active). **PricingSummary UI** updated to show a Calculated / Final pair side by side, with an amber "Adjusted from calculated estimate" badge when an override is active. The summary now optionally accepts `settings` and `finalView` props so it can render the right values without owning override state. **NewQuoteForm** wiring: page loads residential settings via `loadResidentialPricingSettings(supabase)` and forwards them to the form, which forwards them to `calculateQuotePrice`. **Description engine micro-tweaks (Part 11)**: replaced room-checklist scope sentences with broader phrasing ("The clean covers the main areas of the home"); replaced "Add-ons booked alongside this clean:" with "This includes …"; dropped the implicit "linen" reference from the airbnb turnover scope; softened the depth lines that used to enumerate "Skirtings, edges, interior windows…" to broader phrasing. **Tests**: 21 new + 13 existing passing in two files (`residential-pricing-engine.test.ts` covers all 8 Part-13 scenarios + override + capped combine; `quote-wording.test.ts` updated for the new extras phrasing). The pre-existing 5 "7-bed clamp" pricing-test failures remain unrelated. Pricing math, share routes, PDFs, emails, Stripe checkout, proposal templates, invoice totals: untouched. Pending: PR + merge + production validation.
- **Residential service upgrade — wording engine + extras taxonomy** (in progress, same branch). Refactor of `src/lib/quote-wording.ts` and `src/lib/quote-pricing.ts`. Removes the multi-paragraph "Sano 100-Point" template and replaces it with a 3-6 sentence conversational generator: opening → scope summary → condition focus → service-level depth (when applicable) → extras → service-specific closing. New `closingLineFor()` exports the Standard/Deep/EoT/Pre-sale/Post-construction/Airbnb closings per spec. New `state` + `focus` fields on every `ConditionOption` so the prose template "The property is {state}, so the focus will be {focus}" reads naturally for every tag (build-up, vacant, post-renovation, etc.); commercial sites swap subject to "The site is …". Extras list rebuilt around three categories — high_value (oven, fridge, carpet, upholstery, exterior windows, pressure washing, rubbish removal, garage full clean), detail_time (inside cupboards/drawers, inside wardrobes, blinds, full wall wash, high dusting, balcony/deck), condition_based (heavy grease, heavy mould, post-construction residue). New `filterAddonsForServiceType()` hides items that are baseline for the chosen service type (interior windows hidden for deep/EoT/move-in-out/post-construction; inside cupboards/wardrobes hidden for EoT/move-in-out/handover; high dusting hidden for deep/post-construction) so an operator can't double-charge. **Backwards compat preserved**: every legacy addon code (interior_window, glass_doors, wall_spot_cleaning, skirting_detailing, spot_treatment, outdoor_areas, garage_clean, mould_treatment) still resolves to a label/wording so old quote rows render unchanged; the picker is filtered via `deprecated: true` flags. **New service type**: `residential.post_construction` with multiplier 1.5 + heavy buffer; existing pricing multipliers unchanged. **New addon hours added** (NZ benchmarks) for upholstery_cleaning (1.0h), exterior_window (0.75h), pressure_washing (1.5h), rubbish_removal (0.75h), garage_full (2.0h), inside_cupboards (1.5h), inside_wardrobes (1.0h), blinds_shutters (1.0h), full_wall_wash (2.0h), high_dusting (0.75h), balcony_deck (0.75h), heavy_grease (1.5h), post_construction_residue (2.0h). Existing addon hours **unchanged** — no pricing regression on legacy quotes. **QuoteBuilder.tsx** swaps the static `ADDON_OPTIONS.map` for `filterAddonsForServiceType(service_type_code).map`. Stale snapshot tests in `src/__tests__/lib/quote-wording.test.ts` removed (encoded the old multi-paragraph format); replaced by 21 structural assertions in `src/lib/__tests__/quote-wording.test.ts`. Pricing engine, share routes, PDFs, emails, Stripe checkout, proposal templates, fallback `buildServiceDescription()` in doc-helpers: untouched. Pending: PR + merge + production validation.
- **List-view UX consistency — Phase 1 (Invoices parity)** (in progress, same branch). Brings the Invoices list to the same shape as Quotes / Jobs without touching their working code paths. **Display-settings registry** (`src/lib/portal-display-settings.ts`) extended additively with `INVOICE_FIELDS` (invoice_number, client, company, address, status, total, date_issued, due_date, created_at, linked_quote, linked_job) + an `invoices` key on `DisplaySettings`. Defaults render Invoice # / Client / Status / Issued / Due / Total / Linked quote / Linked job out of the box. The merge loader is unchanged in behaviour — old `portal_settings` rows that don't carry the `invoices` key fall through to the default via `mergeEntity`. **Quote registry** also gains `linked_job` / `linked_invoice` keys so the same column-config system can describe what already renders today. **Linked-record fetching** uses the same PostgREST embed pattern as the Jobs list — `source_quote:quotes!quote_id ( id, quote_number )` and `source_job:jobs!job_id ( id, job_number, status )` — so each invoice row carries the quote and job it came from. **Invoices list rebuild**: hard-coded columns replaced with registry-driven render via the same `cell()` dispatch pattern Quotes / Jobs use; whole-row `<Link>` removed in favour of cell-level clickable elements (invoice number, linked quote, linked job, and a small "Open →" affordance on the right). Mobile card view preserved with primary/secondary fields from settings + a one-line linked-record line. **`InvoiceFilters`** new client component (`src/app/portal/invoices/_components/InvoiceFilters.tsx`) — slim search + sort row matching `JobFilters` shape (Newest / Oldest / Due soonest / Due latest / Issued / Invoice #). Search runs across invoice number, client name, company, address, client reference, linked quote number and linked job number. **Settings UI**: `/portal/settings/display` form gains an "Invoices" `<EntitySection>` so an admin can pick which invoice columns are visible, the primary/secondary mobile fields, the default sort, and (where applicable) group-by. **Cleanup mode + `is_test` / archived filtering** preserved unchanged — `BulkSelectProvider` still wraps the table when `canCleanup` is true; show-archived toggle still appears in `ListLifecycleTabs`. Phase 1 deliberately scoped narrow: no shared `<PortalListTable>` primitive yet (Phase 2), no expanded next-action matrix (Phase 3). Pending: PR + merge + production verification.
- **List-view UX consistency — Phase 2 (shared PortalListTable + Jobs migration)** (in progress, same branch). Extracts the common list shell so Quotes / Jobs / Invoices share one primitive instead of three drifting copies. **PR-A** introduces `src/app/portal/_components/PortalListTable.tsx` — a generic server component that owns the desktop table + mobile card switch, the bulk-select column wiring, attention-chip placement under the first column, the row-end "Open →" affordance, and an optional `rowExtraActions` slot for per-entity quick actions. The page still owns data fetching, sort/filter/tabs, the `cell()` dispatcher, the `BulkSelectProvider` wrap (so the action bar mounts at page level), and the per-tab empty states; the primitive only renders. Invoices migrated first as the smallest consumer — Phase 1 behaviour preserved exactly (pixel parity verified). **PR-B** migrates the Jobs list onto the same primitive with the agreed UX deltas: customer-first display rule (`company_name` if set, else client `name`), derived display status via `getJobStatus()` rendering as Needs scheduling / Scheduled / In progress / Completed / Invoiced (DB `jobs.status` unchanged — only the badge label is derived), default visible columns are `Job # | Customer | Address | Value | Scheduled | Contractor | Status | Quote | Invoice` with `value` (job_price) sortable + right-aligned, removed `title` and `company` from defaults (still available in registry), clickable Quote and Invoice chips that deep-link to the related record (`source_quote.id` / `linked_invoice.id` from the embedded select), cell-level navigation throughout (no whole-row `<Link>`). `JOB_STATUS_*` registries gain `needs_scheduling` (amber) and `scheduled` (blue) labels/styles in `src/lib/quote-status.ts`. `JOB_FIELDS` registry extended with `value` / `linked_quote` / `linked_invoice`; default `visibleFields` + `secondaryField` updated. Filters row, Calendar / New Job buttons, cleanup-mode bulk select, archived/test dimming, attention chips, mobile card layout: all preserved. 253/253 lib tests passing; tsc + eslint clean. Pending: PR + merge + production verification.
- **CRM — Airbnb business_type** (✅ committed `5948fc7`, same branch, 2026-04-30). Extends `clients.business_type` CHECK constraint (`docs/db/2026-04-30-clients-business-type-airbnb.sql`, applied via Supabase MCP) and the matching TS `BusinessType` union with an `'airbnb'` value. New Client modal shows "Airbnb / short-stay host" between Body corporate and Other. No payment-default change — Airbnb hosts fall through to the existing individual/company defaults in `defaultPayment()`. Existing six options remain valid; existing rows unchanged. Pending: PR + merge + production verification.
- **5.5.16 — Workflow simplification + action-driven status + data integrity** (in progress, branch `feat/attention-bulk-phase-5-5-14`). Implements the recommendations from the Quote → Job → Invoice flow review. **DB:** `docs/db/2026-04-28-phase-5-5-16-invoice-number-unique.sql` adds a partial UNIQUE index on `invoices.invoice_number` (zero duplicates pre-flight, applied via MCP); `docs/db/2026-04-28-phase-5-5-16-backfill-quote-allowed-hours.sql` backfills `quotes.allowed_hours = estimated_hours` for the 14 existing rows that had it null. **Re-conversion guard** (new `src/lib/quote-conversion-guard.ts` with `assertQuoteConvertible(supabase, quoteId, kind)`): every conversion action — `convertToInvoice`, `createJobFromQuoteWithSetup`, `createJobAndInvoiceFromQuote` — calls it first. The guard refuses if the quote is missing / archived / already converted, OR if a live (non-archived) child of the requested kind already exists. Closes the duplicate-invoice / duplicate-job hole that the cleanup dashboard had been catching after-the-fact. **Job price carry**: `createJobFromQuoteWithSetup` and `createJobAndInvoiceFromQuote` now compute `jobPrice = quote.base_price - quote.discount` and write it on insert — eliminates the post-create Edit step that was needed just to unblock invoicing. **`getJobStatus(job)`** in new `src/lib/job-status.ts` — pure function deriving operational status from the fields that actually drive the workflow (`scheduled_date`, `contractor_id` / `assigned_to`, `started_at`, `completed_at`, `invoice_id`). Stored `jobs.status` is preserved as the audit trail of explicit operator transitions; UI-side filtering now prefers the derived helper. 9 unit tests. **Quote next-step panel** (`QuoteNextStepPanel.tsx`) collapsed from the four-card "menu of four" to a single primary CTA (Create job, sage-bordered, emphasised) plus a "More options" disclosure that reveals Create invoice only / Job + invoice / Recurring as secondary cards. The combined Job + Invoice card is intentionally still reachable for muscle-memory but no longer surfaced upfront — analytics / a follow-up phase will decide whether to remove it entirely. **Job-completed → invoice prompt**: new `JobNextStepCard.tsx` surfaces an emerald "Next step: Create invoice" banner on the job detail page when the job is completed (or `completed_at` is set) and no invoice is linked yet. One click calls `createInvoiceFromJob`; everything prefills from the job. **Action-driven status**: removed the manual status-button row from `JobForm.tsx` (was previously letting an admin flip `draft → assigned → in_progress → completed → invoiced` by hand). Status is now only changed by the dedicated action buttons (Start Job / Complete Job in `JobStatusActions`, plus the conversion actions). **`allowed_hours` first-class on quotes**: `createQuote` writes `allowed_hours = estimated_hours` by default; `createJobFromQuoteWithSetup` reads `quote.allowed_hours` first, falls back to `estimated_hours`; the JobSetup wizard accepts a new `allowedHours` seed prop and prefills accordingly (override per-job still allowed). **Duplicate-quote suspicion** on `/portal/quotes/new`: new `findRecentQuotesForClient` server action returns up to 5 live quotes (last 30 days, status in draft/sent/viewed/accepted) for the chosen client, optionally narrowed by service-address ilike substring; an amber banner renders above the form with deep links + a "Continue anyway" dismiss. Pricing engine, share routes, PDFs, emails, Stripe checkout, proposal templates, recurring jobs path: untouched. Pending: PR + merge + production validation.
- **5.5.14 v2 — Cleanup-mode access control + admin gating** (in progress, same branch). Locks down the lifecycle UI behind a two-key gate. New `workforce_settings.enable_cleanup_mode` boolean (default **false**) — surfaced as a single toggle in Portal access settings, admin-only, with a clear amber-tinted warning panel. New `src/lib/cleanup-mode.ts` exposes `getCleanupAccess(supabase) → { isAdmin, isCleanupModeEnabled, canCleanup, userId }` and a server-action helper `requireCleanupAccess` that returns `{ ok, userId }` or `{ error }`. Both gates are checked: caller must be `michael@sano.nz` AND `enable_cleanup_mode` must be true. Settings load is skipped entirely for non-admins. **Server-side**: `_actions/lifecycle.ts` (`markAsTest` / `archiveRecords` / `restoreRecords` — single AND bulk) all swap the old `requireAdmin` for `requireCleanupAccess`. UI hides are now convenience-only; turning the toggle off blocks the API even for admins. **UI hides**: list pages compute `canCleanup` once per render and use it to (a) not wrap in `BulkSelectProvider`, (b) not render the per-row checkbox column, (c) not render the select-all header, (d) ignore `?show_archived=1` and never render the "Show archived/test" toggle in `ListLifecycleTabs`. Detail pages (quote / job / invoice) gate the per-record `LifecycleActions` panel on `isAdmin && canCleanup`. **Status badge palette** tightened per Part 9: jobs `in_progress` → blue (was amber), `completed` → grey (was emerald) so blue=in-motion and grey=quietly-done are consistent. **Tests**: 9 new unit tests in `cleanup-mode.test.ts` covering both rejection paths (non-admin, admin-with-cleanup-off) and the happy path. All 16 attention-rule tests still pass. (Five failing scenarios in the pre-existing `quote-pricing.test.ts` are unrelated.) Pending: PR + merge + production validation.
- **5.5.14 — Bulk actions + attention system + UX polish** (in progress, branch `feat/attention-bulk-phase-5-5-14`). Builds on the 5.5.13 lifecycle plumbing. New `src/lib/attention-rules.ts` exposes pure helpers `getQuoteAttention`, `getJobAttention`, `getInvoiceAttention` that each return `{ needsAttention, reasons[], nextStep? }`. Reasons map: quotes — `Unsent draft` / `Follow up required` (sent ≥3 days, `SENT_FOLLOWUP_DAYS`) / `Ready for job` (accepted with no downstream job/invoice); jobs — `Needs scheduling` / `Unassigned` / `At risk` (scheduled within 24 h with no contractor) / `Ready to invoice` (completed); invoices — `Not sent` (draft) / `Overdue` / `Outstanding`. Next-step nudges follow the same matrix. **Default tab** on all three list pages flipped to `Needs attention` — a virtual tab that pre-filters by status at the DB level, then the per-row attention rules decide what stays. The four lifecycle tabs from 5.5.13 (Sent / Accepted / All for quotes; Needs scheduling / Scheduled / In progress / Completed for jobs; Outstanding / Paid / Draft / All for invoices) sit alongside it. Quotes page also resolves "has a downstream job/invoice?" once per page-load (single `IN` query against jobs + invoices) so the `Ready for job` chip suppresses the moment the operator acts. New `src/app/portal/_components/AttentionChips.tsx` renders the chip row inline under the primary cell with subtle color coding (gray neutral, amber clock, red alert, emerald done) and an `→ next step` afterword stripped of the leading `Next:`. **Bulk actions UI**: new `src/app/portal/_components/BulkSelect.tsx` provides `BulkSelectProvider` (context wrapper), `BulkSelectCheckbox` (per-row), `BulkSelectHeader` (select-all with indeterminate state), and an auto-mounted sticky action bar — all three lifecycle actions (Mark as test / Archive / Restore) work on selections via the existing `_actions/lifecycle.ts` (which already accepted `string | string[]`). Confirmation modal explains the consequences before each action; success flash shows count. **Jobs filter row simplified**: legacy chip strip (All / Today / Tomorrow / Unassigned / In Progress / Completed) removed — the lifecycle tabs cover the same surface — replaced with a slim search + date dropdown + contractor filter + sort + clear. Dashboard KPI links updated to use the new tab system (`?tab=needs_scheduling` instead of `?view=unassigned`). **Status badge palette** standardised to a 5-tone system: gray neutral (draft) / blue in-motion (sent, assigned) / sky in-motion-soft (viewed) / emerald accepted-or-done (accepted, paid, completed) / sage terminal-success (converted, invoiced) / red declined-or-cancelled / amber problem (in_progress, overdue). Pricing engine, share routes, PDFs, emails, Stripe checkout, proposal templates: untouched. **No DB migration** — all behaviour reads existing columns. Pending: PR + merge + production validation.
- **5.5.13 — Cleanup + lifecycle + UI standardisation** (in progress, branch `feat/lifecycle-cleanup-phase-5-5-13`). Migration `docs/db/2026-04-28-phase-5-5-13-lifecycle-is-test.sql` applied: `quotes` / `jobs` / `invoices` each gain `is_test boolean not null default false`, plus partial indexes `<table>_live_idx (created_at desc) WHERE deleted_at IS NULL AND is_test = false`. Existing `deleted_at` column continues to act as the archive flag (introduced Phase 6). **Live-record rule**: every dashboard, list, and KPI query is now `deleted_at IS NULL AND is_test = false` by default. Test / archived rows reappear only when an admin explicitly opens a list with the new `?show_archived=1` toggle. New `src/app/portal/_actions/lifecycle.ts` exposes `markAsTest`, `archiveRecords`, `restoreRecords` (each accepts a single id or an array, supports bulk) for `quote` / `job` / `invoice`; restore takes an optional `asLive: true` to also clear the test flag. Every transition is admin-gated and audit-logged (`<entity>.marked_test`, `<entity>.archived`, `<entity>.restored`, with `before/after` blobs). New shared `<ListLifecycleTabs>` server component renders consistent tab strips + Show-archived/test toggle on the three list pages, deep-linkable via URL params. **Quotes list** gains `Needs action` (default — drafts + expired) / `Sent` / `Accepted` / `All` tabs with helpful empty states ("No quotes need action. Create a new quote or check the Sent tab to follow up."). **Jobs list** gains `Needs scheduling` (default — no date or no contractor, status draft/assigned) / `Scheduled` / `In progress` / `Completed`; legacy `?view=` KPI shortcuts preserved on top. **Invoices list** gains `Outstanding` (default — sent) / `Paid` / `Draft` / `All`. Each list row carries a small Test or Archived chip when applicable and gets an `opacity-60` dim when the row is not live. New shared `<LifecycleActions>` client component renders three contextual buttons (Mark as test / Archive / Restore — and `Restore as live` for test rows that have been archived) on each entity's detail page, admin-only. Dashboard counts honour the same live-record rule. Pricing engine, commercial-quote flow, share routes, PDFs, emails, Stripe checkout, and proposal templates all unchanged. **Deferred to a follow-up** (called out in output): bulk select UI on the list pages (server actions already accept arrays so this is purely a UX layer), and the visual-consistency status-badge palette refresh.

### Settings additions (planned, 5.5.2+)
Extend `workforce_settings.value` with: `invite_email_subject` (✅ live in 5.5.8 via `/portal/settings/portal-access`), `invite_email_body_template` (✅ live in 5.5.8), `reset_email_subject` (✅ live in 5.5.8), `reset_email_body_template` (✅ live in 5.5.8), `invite_expiry_days` (default 7, planned), `enable_contractor_portal` (default true, ✅ live in 5.5.3 — toggleable in 5.5.8), `enable_pwa_prompt` (default true, ✅ live in 5.5.4 — toggleable in 5.5.8), `contractor_mobile_bottom_nav_enabled` (default true, ✅ live in 5.5.4), `enable_customer_portal` (default false, ✅ live in 5.5.5 — toggleable in 5.5.8), `customer_invite_email_subject`, `customer_invite_email_body_template` (planned). Email body templates support `{{name}}` and `{{link}}` placeholders; `lib/auth-invites.ts` reads these on every send.
