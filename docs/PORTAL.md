...
- **5.5.4 — Mobile-first contractor portal + PWA** (in progress, branch `feat/mobile-pwa-phase-5-5-4`). PWA setup: new `public/manifest.json` (name "Sano", standalone display, `start_url=/contractor/jobs`, theme `#076653`, background `#F7F9F7`, sano-mark.svg + sano-logo-stacked.png as icons), root `app/layout.tsx` extended with `metadata.manifest`, `metadata.icons`, `appleWebApp`, and a `viewport.themeColor` export. New `ContractorBottomNav` (mobile-only, `md:hidden`, fixed bottom, safe-area-inset aware) with three tabs Jobs / Training / Profile and `aria-current` active state. Contractor layout pads `<main>` with `pb-24 md:pb-6` to clear the nav. Jobs list (`/contractor/jobs`) re-grouped into Today / Overdue / Upcoming / Earlier buckets — Today gets emerald ring, Overdue amber ring; cards now show date+time and truncated address with calendar/map icons; refreshed empty state. Job detail (`/contractor/jobs/[id]`): primary action wrapper switches to `fixed` on mobile (sat above the bottom nav at `calc(56px + env(safe-area-inset-bottom))` with white/blur background) and falls back to inline on `md+`; page wrapper bumps to `pb-28 md:pb-8` so the last card isn't covered. New `/contractor/profile` page (read-only contact card + Training shortcut + sign-out button). New `InstallPrompt` component: A2HS captures `beforeinstallprompt` on Android/Chrome and offers an Install button; iOS Safari sees a "Share → Add to Home Screen" hint instead; dismiss persists in localStorage; hidden when already running standalone. Workforce settings extended with `enable_pwa_prompt` (default true) and `contractor_mobile_bottom_nav_enabled` (default true). Pending: proper square 192/512 PNG icons (currently leaning on the SVG mark + stacked PNG); PR + merge.
...
Extend `workforce_settings.value` with: `invite_email_subject`, `invite_email_body_template`, `reset_email_subject`, `reset_email_body_template`, `invite_expiry_days` (default 7), `enable_contractor_portal` (default true, ✅ live in 5.5.3), `enable_pwa_prompt` (default true, ✅ live in 5.5.4), `contractor_mobile_bottom_nav_enabled` (default true, ✅ live in 5.5.4), `enable_customer_portal` (default false).
