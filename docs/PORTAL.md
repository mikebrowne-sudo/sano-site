...
- **5.5.5 — Customer-portal scaffold + PWA icon tidy-up** (in progress, branch `feat/customer-portal-scaffold-phase-5-5-5`). Migration `docs/db/2026-04-27-phase-5-5-5-customer-portal-scaffold.sql` applied: `clients` gains `auth_user_id` (FK → auth.users on delete set null), `invite_sent_at`, `invite_accepted_at`, `access_disabled_at`, `access_disabled_reason`; new partial indexes `clients_auth_user_idx` (where auth_user_id is not null) and `clients_email_lower_idx`. Workforce settings extended with `enable_customer_portal` (default **false**). Middleware (`src/middleware.ts`) gains a `/client/:path*` matcher: anonymous users hitting a client route bounce to `/portal/login`; contractor users hitting `/client/*` are sent to `/contractor/jobs`; staff users (no client row) are sent to `/portal`; client users (`clients.auth_user_id = auth.uid()`) are allowed through. Login redirects extended so client users land on `/client/dashboard`. Placeholder pages: `/client` redirects to `/client/dashboard`; `/client/dashboard` is a server component that reads `loadWorkforceSettings(supabase)` and renders one of two messages — "Your Sano client portal is coming soon." (when `enable_customer_portal=true`) or "Client portal access is not available yet." (when false). No client data is queried or rendered yet. **Open RLS risk noted**: pre-existing `Public can read clients for shared docs` policy is `using true` for SELECT — must be scoped or replaced with a `clients_self_read` policy before any real customer-facing data is rendered (tracked as the blocking item for any future 5.5.6+ customer UI). PWA icon tidy-up: new `scripts/generate-pwa-icons.mjs` (Node + sharp dev-dep) renders `public/brand/sano-mark.svg` onto a sage-50 background at 192×192, 512×512, and a 60%-safe-zone maskable 512×512; commits `public/icons/icon-192.png`, `icon-512.png`, `icon-maskable-512.png`. Manifest re-pointed at the proper PNG icons; root layout's `apple` icon switched from the non-square logo asset to `icon-192.png`. Pending: PR + merge.
