...
- **5.5.8 — Settings UI + email template wiring + client PDF actions** (✅ shipped, merged to main via PR #100). `lib/auth-invites.ts` loads `invite_email_*` / `reset_email_*` from `workforce_settings` and passes them to `sendInviteEmail` / `sendResetEmail` as overrides; `lib/resend.ts` interpolates `{{name}}` / `{{link}}`. New `/portal/settings/portal-access` form (feature flags + email templates). `/client/dashboard` got `View` / `PDF` buttons; the PDF variant appends `?print=1` and a new `AutoPrint` client island fires `window.print()` once.
- **5.5.9 — CRM data foundation (additive + backfill)** (DB applied via MCP; branch `feat/crm-foundation-phase-5-5-9` open, not yet merged). Migration `docs/db/2026-04-27-phase-5-5-9-crm-foundation-schema.sql` + `…-backfill.sql` applied. `clients` gained `client_type` / `lead_source` / `referred_by` / `accounts_email` / `is_archived` / `archived_at`. New `contacts` and `sites` tables (each with Staff full + `*_self_read` RLS). Document tables gained nullable `contact_id` + `site_id`; `jobs` gained `source` (quote/manual/recurring); `invoices` gained `job_id` (the missing reverse link) + `source` (job/manual/recurring). Backfill: every client got a primary contact + a primary site; quotes/jobs/invoices linked to those primaries; jobs gained `source` (8 quote, 2 manual); 7 invoices gained `job_id` from the unambiguous reverse `jobs.invoice_id` link. **Soft warning surfaced**: `JOB-0002` has `client_id ≠ quote.client_id` (carried into 5.5.10).
