...
- **5.5.8 — Settings UI + email template wiring + client PDF actions** (in progress, branch `feat/settings-email-pdf-phase-5-5-8`). **Email wiring**: `lib/auth-invites.ts` now loads the four email-template fields via `loadWorkforceSettings(getServiceSupabase())` (with try/catch fallback) and passes them through to `sendInviteEmail` / `sendResetEmail` as `subjectOverride` + `bodyTemplateOverride`. `lib/resend.ts` adds `applyTemplate` (trivial `{{name}}` / `{{link}}` substitution) and `templateToHtml` (escape + paragraph-break + auto-anchor on the link); when the override body is non-blank it replaces the inline copy inside the same branded `authEmailFrame` (which now takes `bodyHtml`); when blank it falls back to the existing default. Subject overrides work the same. Affects every transactional email path that goes through `inviteUser` (staff, contractor, client invites — all branches converge on this helper) and every `requestPasswordReset`. **Settings page**: new `/portal/settings/portal-access` (admin-only — non-admins see a one-line message, no form). Uses an existing pattern: it's added to the existing settings hub at `/portal/settings` with a new "Portal access" card (KeyRound icon). Form has two sections — Feature flags (toggles for `enable_contractor_portal`, `enable_customer_portal`, `enable_pwa_prompt`) and Email templates (subject + body for invite + reset, with `{{name}} / {{link}}` helper text). Save action in `_actions.ts` re-loads the current `workforce_settings`, merges only the flagged keys, and upserts on `key='default'` (preserving every other workforce setting). Audit row written as `settings.updated`. **Client PDF actions**: `/client/dashboard` replaces the trailing chevron with explicit `View` (sage-50 button) and `PDF` (sage-500 button) actions for each quote and invoice. Both open the existing `/share/quote/[token]` or `/share/invoice/[token]` route in a new tab; the PDF variant appends `?print=1`. New `src/app/share/_components/AutoPrint.tsx` is a tiny client island that calls `window.print()` once on mount when `?print=1` is present, so the user gets the browser's Save-as-PDF dialog automatically. No new RLS, no new tables, no share-link changes. Pending: PR + merge.
