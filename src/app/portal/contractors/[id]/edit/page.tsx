...
export default async function EditContractorPage({ params }: { params: { id: string } }) {
  ...
  const { data: contractor, error } = await supabase
    .from('contractors')
    .select('id, full_name, email, phone, hourly_rate, base_hourly_rate, loaded_hourly_rate, holiday_pay_percent, status, worker_type, notes, start_date, end_date, pay_frequency, standard_hours, holiday_pay_method, ird_number, tax_code, ir330_received, kiwisaver_enrolled, kiwisaver_status, kiwisaver_ks3_provided, kiwisaver_optout_filed, kiwisaver_employee_rate, kiwisaver_employer_rate, insurance_provider, insurance_policy_number, insurance_expiry, insurance_liability_cover, company_name, business_structure, nzbn, company_number, legal_name, gst_registered, gst_number, gst_effective_date, gst_end_date, tax_treatment, kiwisaver_rate_source, kiwisaver_rate_effective_date, kiwisaver_temp_reduction_expiry, employment_type, bank_account_name, bank_account_number, payment_terms_days, contract_signed_date, right_to_work_required, right_to_work_expiry, service_areas, approved_services, availability_notes, has_vehicle, provides_own_equipment, key_holding_approved, alarm_access_approved, pet_friendly, experience_level, can_lead_jobs, can_work_solo, can_supervise_others, invite_sent_at, portal_access_active, auth_user_id, preferred_name, address, date_of_birth, emergency_contact_name, emergency_contact_phone, emergency_contact_relationship, id_sighted')
    .eq('id', params.id)
    .single()
