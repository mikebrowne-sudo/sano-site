...
export default async function ContractorDetailPage({ params }: { params: { id: string } }) {
  ...
  const [
    ...
    // KiwiSaver audit trail (admin-read RLS — empty for non-admins).
    supabase
      .from('worker_kiwisaver_events')
      .select('id, event_type, evidence_ref, effective_date, note, performed_at')
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            href={`/portal/contractors/${params.id}/setup`}
            className="inline-flex items-center gap-2 bg-white border border-sage-200 text-sage-700 font-medium px-4 py-2.5 rounded-lg text-sm hover:bg-sage-50 transition-colors"
          >
      ...
      <div className="max-w-2xl space-y-8">
        ...
        {/* KiwiSaver — employees only. Interactive: status + audit record + gated
            transitions (KS2, info pack, two opt-out routes, savings suspension,
            non-operative intention note). */}
        {workerType !== 'contractor' && (() => {
          const c = contractor as Record<string, unknown>
          const ksStatus = (c.kiwisaver_status as string | null) ?? null
          ...
          const win = ksStatus === 'auto_enrolled'
            ? kiwiSaverOptOutStatus({ startDate: (c.start_date as string | null) ?? null, optOutFiled: !!(c.kiwisaver_optout_filed as boolean) }, today)
            : null
          const winText = win && ({
            before_window: win.daysUntilOpen != null ? `KS10 opt-out window opens in ${win.daysUntilOpen} day${win.daysUntilOpen === 1 ? '' : 's'} (received on/after day 14)` : 'KS10 opt-out window not open yet',
            in_window: `KS10 opt-out window open${win.daysLeft != null ? ` — ${win.daysLeft} day${win.daysLeft === 1 ? '' : 's'} left` : ''}`,
            after_window: 'KS10 opt-out window closed — a late opt-out now goes via IRD',
          ...
          return (
            <Section title="KiwiSaver">
              <KiwiSaverPanel
                workerId={contractor.id as string}
                isAdmin={isAdmin}
                status={ksStatus}
                statusLabel={ksLabel}
                enrolled={!!contractor.kiwisaver_enrolled}
                employeeRate={(contractor.kiwisaver_employee_rate as number | null) ?? (contractor.kiwisaver_enrolled ? KS_DEFAULT_EMPLOYEE : null)}
                employerRate={(contractor.kiwisaver_employer_rate as number | null) ?? (contractor.kiwisaver_enrolled ? KS_DEFAULT_EMPLOYER : null)}
                ks2Completed={!!(c.kiwisaver_ks2_completed as boolean)}
                ks2CompletedDate={(c.kiwisaver_ks2_completed_date as string | null) ?? null}
                autoEnrolmentDate={(c.kiwisaver_auto_enrolment_date as string | null) ?? null}
                infoPackDeliveredDate={(c.kiwisaver_info_pack_delivered_date as string | null) ?? null}
                ks10SignedDate={(c.kiwisaver_ks10_signed_date as string | null) ?? null}
                ks10ReceivedDate={(c.kiwisaver_ks10_received_date as string | null) ?? null}
                optoutSubmittedToIrdDate={(c.kiwisaver_optout_submitted_to_ird_date as string | null) ?? null}
                irdApprovalReference={(c.kiwisaver_ird_approval_reference as string | null) ?? null}
                irdApprovalDate={(c.kiwisaver_ird_approval_date as string | null) ?? null}
                payrollStopEffectiveDate={(c.kiwisaver_payroll_stop_effective_date as string | null) ?? null}
                suspensionRef={(c.kiwisaver_savings_suspension_ref as string | null) ?? null}
                suspensionFrom={(c.kiwisaver_savings_suspension_from as string | null) ?? null}
                suspensionTo={(c.kiwisaver_savings_suspension_to as string | null) ?? null}
                intentionNote={(c.kiwisaver_optout_intention_note as string | null) ?? null}
                intentionRecordedAt={(c.kiwisaver_optout_intention_recorded_at as string | null) ?? null}
                winText={winText ?? null}
                winOpen={win?.status === 'in_window'}
                events={(ksEvents ?? []) as KiwiSaverPanelEvent[]}
              />
            </Section>
          )
        })()}
