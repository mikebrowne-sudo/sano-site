...
export default async function ContractorsPage() {
  ...
  if (error) {
    return (
      <div>
        <h1 className="text-2xl font-bold text-sage-800 mb-6">Contractors</h1>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-sage-800">Contractors</h1>
        <Link
      ...
      {rows.length === 0 ? (
        ...
        <div className="bg-white rounded-xl border border-sage-100 overflow-hidden">
              ...
              <tbody>
                {rows.map((c) => (
                  <tr key={c.id} className="border-b border-sage-50 last:border-0 group">
                    <td className="p-0"><Link href={`/portal/contractors/${c.id}`} className="block px-5 py-3 group-hover:bg-sage-50/50 transition-colors font-medium text-sage-800">{c.full_name}</Link></td>
          ...
          <div className="md:hidden divide-y divide-sage-100">
            {rows.map((c) => (
              <Link key={c.id} href={`/portal/contractors/${c.id}`} className="block px-4 py-4 hover:bg-sage-50/50 transition-colors">
