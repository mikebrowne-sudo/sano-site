...
export default async function ContractorsPage() {
  ...
  const { data: contractors, error } = await supabase
    .from('contractors')
    .select('id, full_name, email, phone, hourly_rate, status, worker_type, employment_type, onboarding_status')
    .order('full_name')
              ...
              <tbody>
                {rows.map((c) => (
                  <tr key={c.id} className="border-b border-gray-50 last:border-0 group">
                    ...
                    <td className="p-0"><Link href={`/portal/contractors/${c.id}`} className="block px-5 py-3 group-hover:bg-gray-50 transition-colors"><span className={clsx('inline-block px-2.5 py-0.5 rounded-full text-xs font-medium capitalize',
                      c.status === 'active'     ? 'bg-emerald-50 text-emerald-700'
                      : c.status === 'onboarding' ? 'bg-amber-50 text-amber-700'
                      :                             'bg-gray-100 text-gray-600',
                    )}>{c.status}</span></Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
