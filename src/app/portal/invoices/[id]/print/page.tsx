...
export default async function PrintInvoicePage({ params }: { params: { id: string } }) {
  ...
  const subtotal = (invoice.base_price ?? 0) + addonsTotal - (invoice.discount ?? 0)
  const gst = invoice.gst_included ? subtotal * 3 / 23 : subtotal * 0.15
  const total = invoice.gst_included ? subtotal : subtotal + gst
        ...
        <div className="print-page">
              ...
              <table className="print-meta-table">
                <tbody>
                  ...
                  {invoice.scheduled_clean_date && (
                    <tr><td className="print-meta-label">Scheduled clean</td><td className="print-meta-value">{fmtDate(invoice.scheduled_clean_date)}</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          <section className="print-section">
            ...
            <table className="print-pricing">
              ...
              <tbody>
                <tr>
                  ...
                  <td className="print-pricing-td print-amount">{fmt(invoice.base_price ?? 0)}</td>
                </tr>
                {addons.map((a, i) => (
                  <tr key={i}>
            ...
            <div className="print-totals-box">
              <div className="print-totals-row"><span>Subtotal</span><span>{fmt(invoice.gst_included ? subtotal - gst : subtotal)}</span></div>
          ...
          <section className="print-section">
            ...
            <table className="print-detail-table">
              <tbody>
                <tr><td className="print-detail-label">Bank</td><td className="print-detail-value">Sano Property Services Limited</td></tr>
                <tr><td className="print-detail-label">Account</td><td className="print-detail-value">12-3627-0005597-00</td></tr>
                <tr><td className="print-detail-label">Reference</td><td className="print-detail-value">{invoice.invoice_number}</td></tr>
              </tbody>
            </table>
          </section>
...
const PRINT_CSS = `
  ...
  .print-page {
    max-width: 210mm; margin: 32px auto;
  ...
  .print-header {
    ...
    margin-bottom: 40px; padding-bottom: 24px; border-bottom: 2px solid #076653;
  }
  ...
  .print-totals-box {
    margin-top: 16px; margin-left: auto; width: 240px;
    border: 1px solid #e0eae3; border-radius: 8px;
  ...
  .print-totals-total {
    ...
    background: #e8f5e9; font-weight: 700; font-size: 12pt; color: #1a1a1a;
    font-variant-numeric: tabular-nums;
  }
