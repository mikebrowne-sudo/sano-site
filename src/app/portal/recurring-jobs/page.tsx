...
export default async function RecurringJobsPage() {
  ...
  if (error) {
    return (
      <div>
        <h1 className="text-2xl font-bold text-sage-800 mb-6">Recurring Jobs</h1>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-sage-800">Recurring Jobs</h1>
      ...
      {rows.length === 0 ? (
        ...
        <div className="bg-white rounded-xl border border-sage-100 overflow-hidden">
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.id} className="border-b border-sage-50 last:border-0 group">
                    <td className="p-0"><Link href={`/portal/recurring-jobs/${row.id}`} className="block px-5 py-3 group-hover:bg-sage-50/50 transition-colors font-medium text-sage-800">{row.title}</Link></td>
                    <td className="p-0"><Link href={`/portal/recurring-jobs/${row.id}`} className="block px-5 py-3 group-hover:bg-sage-50/50 transition-colors text-sage-700">{row.clientName}</Link></td>
                    <td className="p-0"><Link href={`/portal/recurring-jobs/${row.id}`} className="block px-5 py-3 group-hover:bg-sage-50/50 transition-colors text-sage-600 capitalize">{row.frequency}</Link></td>
                    <td className="p-0"><Link href={`/portal/recurring-jobs/${row.id}`} className="block px-5 py-3 group-hover:bg-sage-50/50 transition-colors text-sage-600">{row.assignedTo || <span className="text-sage-300">Unassigned</span>}</Link></td>
                    <td className="p-0"><Link href={`/portal/recurring-jobs/${row.id}`} className="block px-5 py-3 group-hover:bg-sage-50/50 transition-colors text-sage-600">{fmtDate(row.nextDue)}</Link></td>
                    <td className="p-0"><Link href={`/portal/recurring-jobs/${row.id}`} className="block px-5 py-3 group-hover:bg-sage-50/50 transition-colors"><span className={clsx('inline-block px-2.5 py-0.5 rounded-full text-xs font-medium capitalize', row.status === 'active' ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600')}>{row.status}</span></Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="md:hidden divide-y divide-sage-100">
            {rows.map((row) => (
              <Link key={row.id} href={`/portal/recurring-jobs/${row.id}`} className="block px-4 py-4 hover:bg-sage-50/50 transition-colors">
