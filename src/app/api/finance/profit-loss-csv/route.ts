...
export async function GET(request: Request) {
  ...
  const [{ data: invoices }, { data: payables }, { data: expenses }] = await Promise.all([
    ...
    supabase.from('contractor_invoices').select('amount, date_paid').eq('status', 'paid'),
  ...
  const contractors = (payables ?? []).map((p) => ({ amount: (p.amount as number | null) ?? 0, datePaid: (p.date_paid as string | null) ?? null }))
  ...
  const rows: Array<Array<string | number>> = [
    ...
    ['Less cost of sales — contractor payments', `${pl.costOfSalesCount} paid`, (-pl.costOfSales).toFixed(2)],
  ...
  if (pl.excludedDoubleCount.length > 0) {
    rows.push([], ['Excluded — already counted in contractor cost of sales', '', ''])
    for (const l of pl.excludedDoubleCount) rows.push([l.label, `${l.count} items`, l.total.toFixed(2)])
  }
