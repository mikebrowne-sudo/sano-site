...
function fakeSupabase(pending: Array<{ id: string; rank: string; company: string; email: string }>) {
  ...
  const client = {
    from(table: string) {
      ...
      if (table === 'sales_campaign_recipients') {
        return {
          select: () => ({ eq: () => ({ eq: async () => ({
            data: pending.map((p) => ({ id: p.id, token: 't-' + p.id, status: 'pending', lead: { id: 'l-' + p.id, company: p.company, contact_name: 'X', email: p.email, status: 'new', unsubscribed_at: null, quality_rank: p.rank } })),
            error: null,
          }) }) }),
...
describe('sendCampaignBatch — A→B→C order + daily limit', () => {
  ...
  it('limit = Infinity sends everything (remaining 0)', async () => {
    const { client } = fakeSupabase([
      { id: '1', rank: 'A', company: 'A', email: 'a@x.co' },
      { id: '2', rank: 'B', company: 'B', email: 'b@x.co' },
    ])
